...
void Compass::set_direction(const float direction) {
   // _direction = float(int(direction) % 360) + float(direction - float(int(direction)));
    //ui->lb_angle->setText(QString::number(_direction) + QApplication::translate("Compass", "\302\260", nullptr));
   // ui->lb_angle->setText(QApplication::translate("Compass", "\302\260", nullptr));
    _direction = float(-1*direction+90);
    update();
}
...
//    };
    ...
    for (int i = 0; i < 18; ++i) {
       ...
       // painter.rotate(-90.0);
        ...
        if(i)
        {
            painter.drawText(QRect(-19, -7, 38, 14), Qt::AlignCenter, QString::number(360-i * 20));
        }
        else
        {
